...
def upsample_mesh(vertices, normals, faces, displacement_map, texture_map,
    ...
    pixel_3d_normals = pixel_3d_normals / np.linalg.norm(
        pixel_3d_normals, axis=-1)[:, np.newaxis]
...
def plot_kpts(image, kpts, color='r'):
    ...
    for i in range(kpts.shape[0]):
        ...
        if i in end_list:
            continue
        ed = kpts[i + 1, :2]
        image = cv2.line(image, (int(st[0]), int(st[1])),
                         (int(ed[0]), int(ed[1])), (255, 255, 255), radius)
...
def tensor_vis_landmarks(images,
    ...
    for i in range(images.shape[0]):
        ...
        if isScale:
            ...
            predicted_landmark[..., 0] = predicted_landmark[
                ..., 0] * image.shape[1] / 2 + image.shape[1] / 2
            predicted_landmark[..., 1] = predicted_landmark[
                ..., 1] * image.shape[0] / 2 + image.shape[0] / 2
        else:
            predicted_landmark = predicted_landmarks[i]
