...
class Evaluator:
    ...
    def __init__(self, opt, projection_mode='orthogonal'):
        ...
        cuda = torch.device(
            'cuda:%d' %
            opt.gpu_id) if torch.cuda.is_available() else torch.device('cpu')
    ...
    def eval(self, data, use_octree=False):
        ...
        with torch.no_grad():
            ...
            if self.netC:
                self.netC.eval()
            save_path = '%s/%s/result_%s.obj' % (opt.results_path, opt.name,
                                                 data['name'])
            if self.netC:
                gen_mesh_color(opt,
                               self.netG,
...
if __name__ == '__main__':
    ...
    test_images = [
        ...
        if ('png' in f or 'jpg' in f) and (not 'mask' in f)
    ]
