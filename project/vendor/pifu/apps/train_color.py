...
def train_color(opt):
    ...
    if opt.load_netC_checkpoint_path is not None:
        ...
        netC.load_state_dict(
            torch.load(opt.load_netC_checkpoint_path, map_location=cuda))

    if opt.continue_train:
        if opt.resume_epoch < 0:
            model_path_C = '%s/%s/netC_latest' % (opt.checkpoints_path,
                                                  opt.name)
        else:
            model_path_C = '%s/%s/netC_epoch_%d' % (opt.checkpoints_path,
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            image_tensor, calib_tensor = reshape_multiview_tensors(
                image_tensor, calib_tensor)
            ...
            resC, error = netC.forward(image_tensor,
                                       netG.get_im_feat(),
                                       color_sample_tensor,
                                       calib_tensor,
                                       labels=rgb_tensor)
            ...
            if train_idx % opt.freq_plot == 0:
                print(
                    'Name: {0} | Epoch: {1} | {2}/{3} | Err: {4:.06f} | LR: {5:.06f} | dataT: {6:.05f} | netT: {7:.05f} | ETA: {8:02d}:{9:02d}'
                    .format(opt.name, epoch, train_idx, len(train_data_loader),
                            error.item(), lr, iter_start_time - iter_data_time,
            ...
            if train_idx % opt.freq_save == 0 and train_idx != 0:
                torch.save(
                    netC.state_dict(),
                    '%s/%s/netC_latest' % (opt.checkpoints_path, opt.name))
                torch.save(
                    netC.state_dict(), '%s/%s/netC_epoch_%d' %
        ...
        with torch.no_grad():
            ...
            if not opt.no_num_eval:
                ...
                test_color_error = calc_error_color(opt, netG, netC, cuda,
                                                    test_dataset, 100)
                ...
                train_color_error = calc_error_color(opt, netG, netC, cuda,
                                                     train_dataset, 100)
            ...
            if not opt.no_gen_mesh:
                ...
                for gen_idx in tqdm(range(opt.num_gen_mesh_test)):
                    ...
                    gen_mesh_color(opt, netG, netC, cuda, train_data,
                                   save_path)
                train_dataset.is_train = True
