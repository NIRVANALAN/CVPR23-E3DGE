...
def load_trimesh(root_dir):
    ...
    for i, f in enumerate(folders):
        ...
        meshs[sub_name] = trimesh.load(
            os.path.join(root_dir, f, '%s_100k.obj' % sub_name))
...
class TrainDataset(Dataset):
    ...
    def __init__(self, opt, phase='train'):
        ...
        self.aug_trans = transforms.Compose([
            transforms.ColorJitter(brightness=opt.aug_bri,
    ...
    def get_subjects(self):
        ...
        var_subjects = np.loadtxt(os.path.join(self.root, 'val.txt'),
                                  dtype=str)
    ...
    def get_render(self,
        ...
        view_ids = [
            self.yaw_list[(yid + len(self.yaw_list) // num_views * offset) %
                          len(self.yaw_list)] for offset in range(num_views)
        ]
        if random_sample:
            view_ids = np.random.choice(self.yaw_list,
                                        num_views,
                                        replace=False)
        ...
        for vid in view_ids:
            param_path = os.path.join(self.PARAM, subject,
                                      '%d_%d_%02d.npy' % (vid, pitch, 0))
            render_path = os.path.join(self.RENDER, subject,
                                       '%d_%d_%02d.jpg' % (vid, pitch, 0))
            mask_path = os.path.join(self.MASK, subject,
                                     '%d_%d_%02d.png' % (vid, pitch, 0))
            ...
            if self.is_train:
                ...
                if self.opt.aug_blur > 0.00001:
                    blur = GaussianBlur(np.random.uniform(
                        0, self.opt.aug_blur))
                    render = render.filter(blur)

            intrinsic = np.matmul(trans_intrinsic,
                                  np.matmul(uv_intrinsic, scale_intrinsic))
    ...
    def select_sampling_method(self, subject):
        ...
        random_points = np.random.rand(
            self.num_sample_inout // 4,
            3) * length + self.B_MIN  # * uniform points in 3D
        ...
        inside_points = inside_points[:self.num_sample_inout //
                                      2] if nin > self.num_sample_inout // 2 else inside_points
        outside_points = outside_points[:self.num_sample_inout //
                                        2] if nin > self.num_sample_inout // 2 else outside_points[:(
    ...
    def get_color_sampling(self, subject, yid, pid=0):
        ...
        pitch = self.pitch_list[pid]
        uv_render_path = os.path.join(self.UV_RENDER, subject,
                                      '%d_%d_%02d.jpg' % (yaw, pitch, 0))
        uv_mask_path = os.path.join(self.UV_MASK, subject, '%02d.png' % (0))
        uv_pos_path = os.path.join(self.UV_POS, subject, '%02d.exr' % (0))
        uv_normal_path = os.path.join(self.UV_NORMAL, subject,
                                      '%02d.png' % (0))
        ...
        if self.num_sample_color:
            sample_list = random.sample(range(0, surface_points.shape[0] - 1),
                                        self.num_sample_color)
        ...
        return {'color_samples': samples, 'rgbs': rgbs_color}
    ...
    def get_item(self, index):
        ...
        render_data = self.get_render(subject,
