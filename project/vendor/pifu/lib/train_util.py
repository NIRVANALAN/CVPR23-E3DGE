...
def reshape_multiview_tensors(image_tensor, calib_tensor):
    ...
    image_tensor = image_tensor.reshape(
        ...
        image_tensor.shape[3], image_tensor.shape[4])
    calib_tensor = calib_tensor.reshape(
        calib_tensor.shape[0] * calib_tensor.shape[1], calib_tensor.shape[2],
...
def gen_mesh(opt, net, cuda, data, save_path, use_octree=True):
    ...
    calib_tensor = data['calibs'].to(device=cuda)
    ...
    try:
        ...
        verts, faces, _, _, reconstructed_sdf = reconstruction(
            net,
            cuda,
            calib_tensor,
            opt.resolution,
        ...
        np.save(f'{save_path}_sdf.npy', reconstructed_sdf)
...
def gen_mesh_color(opt, netG, netC, cuda, data, save_path, use_octree=True):
    ...
    try:
        ...
        verts, faces, _, _ = reconstruction(netG,
                                            cuda,
                                            calib_tensor,
                                            opt.resolution,
                                            b_min,
...
def calc_error_naive(opt, net, cuda, dataset_dict, num_tests):
    # to merge with dataclass dataset. use dict for now.
