# https://github.com/omertov/encoder4editing/blob/main/scripts/calc_losses_on_images.py
...
from project.losses.lpips.lpips import LPIPS
...
def run(args):
    ...
    if args.mode == 'lpips':
        loss_func = LPIPS(device='cuda', net_type='alex')
    elif args.mode == 'l2':
    ...
    for result_batch, gt_batch in tqdm(dataloader):
        for i in range(args.batch_size):
            loss = float(
                loss_func(result_batch[i:i + 1].cuda(),
                          gt_batch[i:i + 1].cuda()))
    ...
    with open(os.path.join(out_path, 'stat_{}.txt'.format(args.mode)),
              'w') as f:
        f.write(result_str)
    with open(os.path.join(out_path, 'scores_{}.json'.format(args.mode)),
              'w') as f:
